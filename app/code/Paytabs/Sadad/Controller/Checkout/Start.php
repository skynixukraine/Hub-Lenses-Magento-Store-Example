...
namespace Paytabs\Sadad\Controller\Checkout;
...
class Start extends \Magento\Framework\App\Action\Action
{
    ...
    */
    ...
    \Paytabs\Sadad\Model\PaymentMethod $paymentMethod,
    ...
    */
    public function execute()
    {
		...
		$acuid = '<label style="color:#fb0505;">'."SADAD Account ID is required...".'</label>&nbsp;<a href="'.$url.'">Go to Home</a>';
		if(!isset($_GET['acuid']) || trim($_GET['acuid'])=='')
			return $result->setData(['html' => $acuid]);
		$acuid = trim($_GET['acuid']);
		
		$retobj = $this->_paymentMethod->getPostHTML($this->getOrder(),$acuid);
		
		if($retobj['response_code'] == '4025')
		{
			$html = '<script language="javascript">window.location.href = "'.$retobj['payment_url'].'"</script>';
			$html .='<a href="'.$retobj['payment_url'].'">Please Wait... Redirecting to SADAD Payment...</a>';
