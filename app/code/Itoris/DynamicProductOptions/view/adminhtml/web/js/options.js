...
define([
    ...
    Itoris.OptionManager.prototype = {
        ...
        createItemRow : function(config, parentBlock) {
            ...
            this.prepareExtraAction(row, config);
        ...
        prepareExtraAction: function(row, config) {
            var addExtraElm = row.select('.edit-value-extra')[0];
            var hasExtraElm = row.select('.dynamicoptions-has-extra')[0];
            var isHasExtra = config.separate_cart_item || config.hide_sku || config.qty_multiplier || config.special_price || (config.gallery_images && config.gallery_images.length);
            hasExtraElm.update(isHasExtra ? '&#10004;' : '');
            addExtraElm.observe('click', this.showExtraPopup.bind(this, row, config));
        },
        addProductLinkToItem : function(productData) {
            ...
            this.createThumbnail(productData.image_url, this.productGridItemRow.select('input.dynamicoptions-value-image_src')[0], 'image_src', this.productGridItemConfig);
            var fieldType = this.productGridItemRow.up('.config').getAttribute('field-type');
            if (fieldType == 'checkbox' || fieldType == 'radio') {
                this.productGridItemRow.select('input.dynamicoptions-value-swatch')[0].checked = true;
                this.productGridItemConfig.swatch = 1;
            }
        },
        ...
        uploadGalleryImage: function(input){
            this.popup.loadingMask.show();
            var clone = input.clone(true), that = this;
            var iframeName = Math.random()+'_uploader', iframe = jQuery('<iframe name="'+iframeName+'" style="display:none"></iframe>');
            var form = jQuery('<form enctype="multipart/form-data" action="'+ encodeURI(this.config.upload_image_url) + '" style="display:none" method="post" target="'+iframeName+'"><input type="hidden" name="form_key" value="'+FORM_KEY+'"></form>');
            jQuery(input).parent().append(iframe).append(form).append(clone);
            form.append(input);
            iframe.on('load', function(){
                var iframeDocument = iframe[0].contentDocument ? iframe[0].contentDocument : iframe[0].contentWindow.document;
                if (iframeDocument.getElementById('image_src')) {
                    var imageSrc = iframeDocument.getElementById('image_src').innerHTML;
                    jQuery('<span class="gallery_image">').prop('_src', imageSrc).append(
                        jQuery('<img>').prop({'src': imageSrc})
                    ).append(
                        jQuery('<span class="remove-img-icon"></span>').click(function(){
                            jQuery(this).closest('.gallery_image').remove();
                        })
                    ).appendTo(jQuery('#itoris_dynamicoptions_extra_popup .gallery_images'));
                } else if (iframeDocument.getElementById('error')) {
                    alert(iframeDocument.getElementById('error').innerHTML)
                }
                jQuery(form).remove();
                jQuery(iframe).remove();
                that.popup.loadingMask.hide();
            });
            form[0].submit();
        },
        ...
        parseMediaVariables: function(str) {
            str = str.replace(/{{media url=&quot;/g, '{{media url="').replace(/&quot;}}/g, '"}}');
        ...
        showSwatchHtml: function(row, config, addSwatchHtmlElm) {
            ...
            toggleBtn.after('<i style="margin-left:20px">FYI: You can use variables {title}, {price}, {image}, and {tooltip}</i>');
        ...
        showExtraPopup: function(row, config) {
            window.dpo_extra_popup = window.dpo_extra_popup ? window.dpo_extra_popup : $('itoris_dynamicoptions_extra_popup');
            window.dpo_extra_popup.config = config;
            row.select('.dynamicoptions-extra-popup-container')[0].appendChild(window.dpo_extra_popup);
            jQuery('#itoris_dynamicoptions_extra_popup .gallery_images').sortable();

            window.dpo_extra_popup.select('.dynamicoptions-value-separate_cart_item')[0].checked = config.separate_cart_item && config.sku_is_product_id ? true : false;
            window.dpo_extra_popup.select('.dynamicoptions-value-separate_cart_item')[0].disabled = !config.sku_is_product_id ? true : false;
            window.dpo_extra_popup.select('.dynamicoptions-value-hide_sku')[0].checked = config.hide_sku ? true : false;
            window.dpo_extra_popup.select('.dynamicoptions-value-qty_multiplier')[0].value = config.qty_multiplier ? config.qty_multiplier - 0 : '';
            window.dpo_extra_popup.select('.dynamicoptions-value-qty_multiplier')[0].disabled = config.use_qty ? false : true;
            window.dpo_extra_popup.select('.dynamicoptions-value-special_price')[0].value = config.special_price ? (config.special_price - 0).toFixed(2) : '';
            window.dpo_extra_popup.select('.gallery_images')[0].innerHTML = '';
            if (config.gallery_images && config.gallery_images.length) {
                config.gallery_images.each(function(imageSrc){
                    jQuery('<span class="gallery_image">').prop('_src', imageSrc).append(
                        jQuery('<img>').prop({'src': imageSrc})
                    ).append(
                        jQuery('<span class="remove-img-icon"></span>').click(function(){
        ...
        saveExtra: function(button) {
            window.dpo_extra_popup.config.separate_cart_item = window.dpo_extra_popup.select('.dynamicoptions-value-separate_cart_item')[0].checked ? 1 : 0;
            window.dpo_extra_popup.config.hide_sku = window.dpo_extra_popup.select('.dynamicoptions-value-hide_sku')[0].checked ? 1 : 0;
            window.dpo_extra_popup.config.qty_multiplier = parseInt(window.dpo_extra_popup.select('.dynamicoptions-value-qty_multiplier')[0].value) > 0 ? parseInt(window.dpo_extra_popup.select('.dynamicoptions-value-qty_multiplier')[0].value) : '';
            window.dpo_extra_popup.config.special_price = parseFloat(window.dpo_extra_popup.select('.dynamicoptions-value-special_price')[0].value) > 0 ? parseFloat(window.dpo_extra_popup.select('.dynamicoptions-value-special_price')[0].value) : '';
            window.dpo_extra_popup.config.gallery_images = [];
            jQuery('#itoris_dynamicoptions_extra_popup .gallery_image').each(function(){
                window.dpo_extra_popup.config.gallery_images.push(this._src);
            }); console.log(window.dpo_extra_popup.config.gallery_images);
            this.hideExtraPopup();
        },
        hideExtraPopup: function() {
            var flag = window.dpo_extra_popup.up('td').select('.dynamicoptions-has-extra')[0];
            var isHasExtra = window.dpo_extra_popup.config.separate_cart_item || window.dpo_extra_popup.config.hide_sku || window.dpo_extra_popup.config.qty_multiplier || window.dpo_extra_popup.config.special_price || (window.dpo_extra_popup.config.gallery_images && window.dpo_extra_popup.config.gallery_images.length);
            flag.update(isHasExtra ? '&#10004;' : '');
            window.dpo_extra_popup.hide();
        }
    };
