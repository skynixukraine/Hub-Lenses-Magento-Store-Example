...
namespace Paytabs\Paytabsexpress\Model;
...
{
    protected $_code = 'paytabsexpress';
    ...
    */
    ...
    {
		$merchantid = trim($this->getConfigData('merchantid'));
		$secretkey = trim($this->getConfigData('secretkey'));
		$secretsign = trim($this->getConfigData('secretsign'));
		
		$returnUrl = trim(self::getReturnUrl());
		
		$txnid = $order->getIncrementId();	
		$amount = $order->getGrandTotal();	
		$amount = trim(number_format((float)$amount, 2, '.', ''));
		...
		$email = trim($billingAddress->getEmail());
		$street = '';
		...
		$region = trim($billingAddress->getRegion());
		$country_iso2 = trim($billingAddress->getData('country_id'));
		//$countryObj = $this->_countryHelper->loadByCode($country);
		//$country = $countryObj->getName();
		$telephone = trim($billingAddress->getTelephone());
		$title = $firstName." ".$lastName;
		
		$cdetails = $this->getCountryDetails($country_iso2);
        $phoneext = trim($cdetails['phone']);
        
        $country = trim($this->countryGetiso3($country_iso2));
		...
		$html = "<form name='paytabsexp-form' id='paytabsexp-form' method='POST'>
					<div class='buttons'>
						<div class='pull-right'>
							<div style='margin-top:20px; margin-bottom-12px;' class='PT_express_checkout'></div>
						</div>
					</div>
				</form>";
		$js = "<style>#PT_express_checkout_wrap{position: absolute!important; min-height: 650px}</style>
			<link rel=\"stylesheet\" href=\"https://www.paytabs.com/theme/express_checkout/css/express.css\">
	        <script type=\"text/javascript\">
			
			
			jQuery.getScript( \"https://www.paytabs.com/express/express_checkout_v3.js\", function() {
			  function launchPaytab() {	
					...
					Paytabs('#express_checkout').expresscheckout({
						settings:{
							...
							style: {
 								css: 'custom',
 								linktocss: 'https://hublenses.com/css/style.css'
 							}
						},						
						customer_info:{
				...
				var checkoutOrderBtn = jQuery(\"button.btn-checkout\");
    ...
    {
        return $this->_getUrl('paytabsexpress/ipn/callback', $storeId, false);
    }
