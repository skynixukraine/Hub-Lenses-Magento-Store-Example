                    ...
                    foreach ($section['fields'] as $index => &$field) {
                        if ($field) {
                            ...
                            if (!in_array($field['type'], ['image', 'html'])) {
                                //fix for duplicated internal_id
                                if (in_array($field['internal_id'], $internalIdsUsed)) $field['internal_id'] = max($internalIdsUsed)+1;
                                $internalIdsUsed[] = $field['internal_id'];
                            ...
                            if (isset($field['items'])) {
                                ...
                                if ($storeId > 0) {
                                    $_values = [];
                                    if (isset($defaultStoreFields[(int) $field['internal_id']]['items'])) foreach($defaultStoreFields[(int) $field['internal_id']]['items'] as $_value) if ($_value) {
                                ...
                                foreach ($field['items'] as &$value) {
                                    if ($value) {
                                        ...
                                        if (isset($value['special_price']) && floatval($value['special_price']) > 0) {
                                            if (isset($value['price_type'])) $con->insert($res->getTableName('catalog_product_option_type_price'), [
                                                ...
                                                'option_type_id'     => $valueId,
                                                'store_id'             => $storeId,
                                                'price'             => floatval($value['special_price']),
                                                'price_type'         => 'fixed'
                                            ]);
                                            $value['compare_at'] = floatval($value['price']);
                                        } else {
                                            if (isset($value['price_type'])) $con->insert($res->getTableName('catalog_product_option_type_price'), [
                                                'option_type_price_id'         => $this->getMaxDb('catalog_product_option_type_price', 'option_type_price_id'),
                                                'option_type_id'     => $valueId,
